...
public class AnnotVersionMergerNewKey {
		...
		try(BufferedReader inAnnotTargetFile = new BufferedReader(new FileReader(args[0]));
			...
			while ((str = inAnnotTargetFile.readLine()) != null) {
				...
				if(annotTarget.containsKey(geneName)){
					...
					//add @category
					mapTmp.put(arrayLine[3]+"_"+arrayLine[4], str);
				}else{
					//new gene
					mapTmp = new HashMap<>();
					mapTmp.put(arrayLine[3]+"_"+arrayLine[4], str);
...
//					System.out.println("aaaaaaa");
			...
			//now comparison we are going to do it in the using the whole list of genes
...
//					System.out.println(geneName);
					...
					while (it.hasNext()) {
						...
						if(mapTarget==null||!mapTarget.containsKey(goEvi)){
